...
met <- read.csv("E:/BioMac_Lab_2nd_training/Archive/data/metadata2.csv")
str(met) # Check the structure of the metadata

#Basic quality check of the .csv files
...
dds$dexamethasone <- relevel(dds$dexamethasone, ref = "untreated") #$ holding the specific column

# Performing differential expression analysis
dds <- DESeq(dds)  # Correcting the line here

# Extracting the results of the analysis
res <- results(dds)

# Writing the results to a CSV file for further examination
write.csv(res, "E:/BioMac_Lab_2nd_training/Archive/data/test_biomac.csv")
...
write.csv(res0.05.df, "E:/BioMac_Lab_2nd_training/Archive/data/final_test_biomac.csv")
...
write.csv(best_genes, "E:/BioMac_Lab_2nd_training/Archive/data/best_genes.csv")
...
Heatmap(mat.z, cluster_rows = TRUE, cluster_columns = TRUE, column_labels = colnames(mat.z), row_labels = top_genes$Symbol)
...
# top_gse_results <- gse@result[1:20,]  # Select top 20 results based on your criteria
...
# gse_top_10 <- gse
# gse_top_10@result <- top_gse_results[1:10, ] 
# gse_simplified <- simplify(gse_top_10, cutoff = 0.7, by = "p.adjust", select_top = 10)

# # Recalculate pairwise term similarity on the simplified result
# gsee_simplified <- pairwise_termsim(gse_simplified)

# # Plot the enrichment map for the simplified set of categories
# emapplot(gsee_simplified, showCategory = 10)
...
ridgeplot(gse) + 
  ...
  theme(axis.text.y = element_text(size = 7))  # Y axis label size reduced
